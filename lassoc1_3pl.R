...
// [[Rcpp::export]]
List nalc13pl(const arma::mat&u,const arma::mat&indic,const arma::mat& eta,const arma::mat& s,
...
arma::mat new_a=a;
arma::mat a_num=mat(domain,domain,fill::zeros);
arma::cube a_denom=zeros(domain,domain,domain);
...
sgvem_3PLEFA_L1_const1 <- function(u,new_a,new_b,new_c,new_s,eta,xi,Sigma, domain,inite,samp,forgetrate,lbd,indic,
  ...
  xi=eta
  prev_a_num = matrix(0,nrow=domain,ncol=domain)  
  prev_a_denom = array(0,dim=c(domain,domain,domain)) 
  ...
  while(converge==1 && rankMatrix(Sigma) == domain && n < 8000){
    ...
    #update a
    rs3=nalc13pl(u, indic, eta, new_s, new_b, SIGMA, MU, new_a, id_1, prev_a_num, prev_a_denom, dec_st)
    new_a=rs3$new_a
    a_denom=rs3$a_denom
    a_num=rs3$a_num
    
    rs4=palc13pl(u, domain, item, id_1, lbd, eta, new_s, new_a, new_b, SIGMA, MU, dec_st, prev_delta, prev_deriv2)
    new_a=rs4$new_a
    delta=rs4$delta
    deriv2=rs4$deriv2
